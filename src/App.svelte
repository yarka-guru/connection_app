<script>
  ...
  import CredentialsDisplay from './lib/CredentialsDisplay.svelte'
  ...
  onMount(async () => {
    try {
      ...
      errorMessage = `Initialization error: ${err}`
      ready = true
    }
  })
  ...
  async function checkForUpdates() {
    ...
    try {
      ...
      if (updateInfo?.updateAvailable) {
        ...
        setTimeout(() => { updateCheckMessage = '' }, 3000)
      }
    } catch (err) {
  ...
  async function handleSavedConnectionConnect(savedConnection) {
    ...
    try {
      ...
      errorMessage = `${err}`
      connectionStatus = activeConnections.length > 0 ? 'connected' : 'disconnected'
  ...
  async function handleSaveConnection() {
    ...
    try {
      ...
      savedConnections = [...savedConnections.filter(c => c.id !== saved.id), saved]
      showSavePrompt = false
      saveConnectionName = ''
    } catch (err) {
  ...
  async function confirmDelete() {
    ...
    try {
      ...
      savedConnections = savedConnections.filter(c => c.id !== showDeleteConfirm.id)
  ...
  let isAlreadySaved = $derived(
    savedConnections.some(c => c.projectKey === selectedProject && c.profile === selectedProfile)
  )
</script>
    ...
    <div class="main-content">
      ...
      {#if showDeleteConfirm}
        <div class="delete-confirm">
          <span>Delete "{showDeleteConfirm.name}"?</span>
