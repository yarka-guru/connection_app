...
describe('Port Mapping', () => {
  it('should map environment suffix to correct port', () => {
    ...
    testCases.forEach(({ env, expected }) => {
      ...
      const matchedSuffix = allEnvSuffixes.find((suffix) =>
        env.endsWith(suffix),
      )
...
describe('Retry Configuration', () => {
  it('should have valid retry values', () => {
    ...
    assert.ok(
      RETRY_CONFIG.BASTION_WAIT_MAX_RETRIES > 0,
      'Max retries should be positive',
    )
    assert.ok(
      RETRY_CONFIG.BASTION_WAIT_RETRY_DELAY_MS > 0,
      'Retry delay should be positive',
    )
    assert.ok(
      RETRY_CONFIG.PORT_FORWARDING_MAX_RETRIES >= 0,
